...
describe("initCompetition single_match", () => {
    ...
    it("llança error si no hi ha exactament 2 equips", () => {
        expect(() => initCompetition("single_match", { eventId: "e1", teamIds: ["t1"] })).toThrow();
    });
});
...
describe("group stage", () => {
    it("reparteix 8 equips en 2 grups de 4", () => {
        const groups = buildGroups(["a", "b", "c", "d", "e", "f", "g", "h"], 4, seededRng(1));
    ...
    it("genera round-robin complet (3 equips = 3 partits)", () => {
        const matches = generateRoundRobinMatches({ id: "A", teamIds: ["a", "b", "c"] }, "e1", makeId);
        expect(matches).toHaveLength(3);
    });
    it("genera round-robin complet (4 equips = 6 partits)", () => {
        const matches = generateRoundRobinMatches({ id: "A", teamIds: ["a", "b", "c", "d"] }, "e1", makeId);
